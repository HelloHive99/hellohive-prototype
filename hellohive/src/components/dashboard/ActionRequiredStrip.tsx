...
export function ActionRequiredStrip({ workOrders }: { workOrders: WorkOrder[] }) {
  ...
  if (approaching + breached + overdue + pendingAck + unassigned + pendingApproval === 0) return null;
  ...
  return (
    <div className="flex items-center gap-2 flex-wrap bg-neutral-900 border border-slate-800/50 rounded-lg px-4 py-3">
      ...
      {pendingApproval > 0 && (
        <button
          onClick={() => router.push('/work-orders?status=pending-approval')}
          className="flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium bg-purple-500/10 text-purple-400 border border-purple-500/20 hover:bg-purple-500/20 transition-colors"
        >
